...
'''
Prepare the bolsonaro/lula tweets data,
adding custom labels in respect to appreciation/non appreciation
'''
...
#Get appreciation/non appreciation of lula and/or bolsonaro
...
)*0.9)#Occurance of appreciative/non appreciative tweets with lula or bolsonaro

minAppreciativeBoth = int(min(
    len(dfAppreciativeBoth),
    len(dfNonAppreciativeBoth)
)*0.9)#Occurance of appreciative/non appreciative tweets with lula and bolsonaro

#Concatenate the different types of tweets
df = pd.concat([
...
#Label all tweets in 5 custom classes
df["expandedLabel"] = 0
...
df.to_csv(outputPath,sep='|',index=False) #Write to csv