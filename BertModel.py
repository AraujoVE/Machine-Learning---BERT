...
class BertModel():
    '''
    Main Bert model to execute a training, evaluation or testing
    '''
    ...
    #Generate the model, tokenizer and device for BERT
    ...
    #Generate the model, tokenizer and device for RoBERTa
    ...
    def tokenizeSentences(self) -> None:
        ...
        maxTokenSize : int = min(int(1.25*(max([len(self.bertDataFixed.tokenizer.tokenize(sentence)) for sentence in sentences]) + 2)),512)
        for sentence in sentences:
            encodedDict =  self.bertDataFixed.tokenizer.encode_plus( #Encode the sentence
                sentence,                     # Sentence to encode.
                add_special_tokens = True,    # Add '[CLS]' and '[SEP]'
                max_length = maxTokenSize,    # Max size of tokens
                padding='max_length',         # Padding value
